<?php
/**
 * DanceMatrix functions and definitions
 *
...
if ( ! function_exists( 'trinity_setup' ) ) :
	...
	function trinity_setup() {
		 ...
		 * If you're building a theme based on DanceMatrix, use a find and replace
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'trinity' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'trinity_custom_background_args', array(
...
function trinity_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'trinity_content_width', 640 );
}
...
function trinity_scripts() {
	...
	wp_enqueue_script( 'trinity-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
